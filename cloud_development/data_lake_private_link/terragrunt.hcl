...
dependency "bs_infra_data_lake" {
  config_path = "${get_original_terragrunt_dir()}/../data_lake"
}
...
inputs = {
  content_map = {
    data_lake_private_link = {
      from = [dependency.bs_infra_vpc.outputs.content_map.vpc, dependency.bs_infra_data_lake.outputs.content_map.data_lake]
